...
static int
doparse (ifunc, srctypename, srcname)
	int (*ifunc)();
	char *srctypename;
	char *srcname;
{
	...
	char *filename;
{
	...
	switch (i) {
	  ...
	  case 7:			/* system.twmrc */
		sprintf(tmpfilename, "%s/default.twmrc", PIEWMDIR);
		break;
	}
...
{
	while (ptr == len) {
		if (fgets(buff, BUF_LEN, twmrc) == NULL) return(EOF);
		if (!strncmp(buff, "#line", 5)) {
			yylineno = atoi(buff + 6);
			continue;
		} else {
			yylineno++;
		}
...
name_list **
do_colorlist_keyword (keyword, colormode, s)
    int keyword;
    int colormode;
    char *s;
{
...
int
do_color_keyword (keyword, colormode, s)
    int keyword;
    int colormode;
    char *s;
{
...
int
do_pixmap_keyword(keyword, filename)
    int keyword;
    char *filename;
{
