...
LOCAL_LIBRARIES = $(XMULIB) $(EXTENSIONLIB) $(XLIB) $(MATHLIB) -lXpm
       LINTLIBS = $(LINTXMU) $(LINTEXTENSIONLIB) $(LINTXLIB)
        DEFINES =  -DMITMISC -DXTEST -DXSYNC -DXCMISC -DXRECORD -DPEXEXT -DMITSHM -DXIE -DBIGREQS -DXF86VIDMODE -DXF86MISC -DDBE  -DSCREENSAVER  $(LOCAL_DEFINES) $(SIGNAL_DEFINES) $(ULTRIX_DEFINES)

           SRCS = gram.c lex.c deftwmrc.c add_window.c gc.c list.c twm.c 		parse.c menus.c events.c resize.c util.c version.c iconmgr.c 		cursor.c icons.c vdt.c move.c LocPixmap.c
...
parse.o:	 vdt.c
	$(RM) $@
		$(CC) -c $(CFLAGS)  '-DPIEWMDIR="$(PIEWMDIR)"' $*.c
...
install:: piewm
	...
	$(INSTALL) $(INSTALLFLAGS) $(INSTPGMFLAGS)  piewm $(DESTDIR)$(BINDIR)/piewm

install.man:: piewm.man
	@if [ -d $(DESTDIR)$(MANDIR) ]; then set +x; \
	else (set -x; $(MKDIRHIER) $(DESTDIR)$(MANDIR)); fi
	$(INSTALL) $(INSTALLFLAGS) $(INSTMANFLAGS) piewm.man $(DESTDIR)$(MANDIR)/piewm.$(MANSUFFIX)
...
install::
	...
	for i in $(TWMDIR); do if [ -d $(DESTDIR)$$i ]; then \
...
deftwmrc.c:  system.twmrc
	...
	@echo '' >>$@
	@echo 'char *defTwmrc[] = {' >>$@
	sed -e '/^#/d' -e 's/"/\\"/g' -e 's/^/    "/' -e 's/$$/",/' 		system.twmrc >>$@
