...
static void getTWM_FLAGS (Window w, unsigned long *flags);
...
TwmWindow *
AddWindow(Window w, int iconmgr, void *iconp)
{
    ...
    tmp_win->iconmgr = iconmgr;
    tmp_win->iconmgrp = (IconMgr *)iconp;
	...
	tmp_win->sticky = 
	    LookInList(Scr->StickyL, tmp_win->full_name, &tmp_win->class)?1:0;
    ...
    {
	tmp_win->iconify_by_unmapping = iconmgr ? FALSE :
	...
	if (tmp_win->attr.x > AddingX) tmp_win->attr.x = AddingX - tmp_win->frame_bw;
	if (tmp_win->attr.y > AddingY) tmp_win->attr.y = AddingY - tmp_win->frame_bw;
	if (tmp_win->attr.x + tmp_win->attr.width < AddingX) tmp_win->attr.x = AddingX - tmp_win->attr.width + tmp_win->frame_bw;
	if (tmp_win->attr.y + tmp_win->attr.height < AddingY) tmp_win->attr.y = AddingY - tmp_win->attr.height + tmp_win->frame_bw;
...
void
SimulateWinGravity (TwmWindow *tmp)
{
...
static void
getTWM_FLAGS (Window w, unsigned long *flags)
{
    Atom actual_type;
    int actual_format;
    unsigned long nitems, bytesafter;
...
void
SetTWM_FLAGS(TwmWindow *tmp_win)
{
    unsigned long data;
